...
from jumanji.environments.logic.drop_7 import Drop7, Observation
from jumanji.environments.logic.drop_7 import utils as drop_7_utils
...
def make_actor_critic_networks_drop_7(
    drop_7: Drop7,
    num_channels: int,
    policy_layers: Sequence[int],
    value_layers: Sequence[int],
) -> ActorCriticNetworks:
    """Make actor-critic networks for the `Drop7` environment."""
    num_actions = drop_7.action_spec.num_values
...
def make_network_cnn(
    ...
    def network_fn(observation: Observation) -> chex.Array:
        visible_disks = jnp.clip(observation.board, 0) / drop_7_utils.NUM_DISK_VALUES
        hidden_disks = observation.board == drop_7_utils.HIDDEN_DISK
        current_disk = jnp.broadcast_to(
            observation.current_disk[..., None, None] / drop_7_utils.NUM_DISK_VALUES,
            observation.board.shape,
        )
        board = jnp.stack(
            [visible_disks, hidden_disks.astype(float), current_disk],
            axis=-1,
        )
        torso = hk.Sequential(
            [
